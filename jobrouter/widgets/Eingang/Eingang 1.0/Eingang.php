...
class Eingang extends Widget{
	...
	public function getRoles(){
		...
		foreach($currentJobFunctions as $JobFunction){
			$temp = "SELECT processname, steplabel, MAX(startdate) FROM jrincidents WHERE recipient = '" .$JobFunction ."' AND (status = '0' OR status = '1') GROUP BY processname, steplabel ORDER BY MAX(startdate) DESC";
			$result = $JobDB->query($temp);
